...
def read_from_snowflake(sql):
    ...
    # print(df)
    return df
###There is no other way exceptusing spark to write directly into snowflake table from dataframe other than pandas
...
def copy_snowflake_internal_stage_local(csv_filename_with_path):
    ...
    try:
        ...
        cs.execute(sql)
        print(f"copy data from snowflake stage into snowflake {table}:" + str(datetime.datetime.now()))
