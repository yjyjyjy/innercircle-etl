...
def set_session():
    ...
    try:
        ...
        ## local files uploaded to snowflake table with proper validation and rejected rows are stored in temp table for reference
...
def copy_from_file_to_snowflake_table(csv_filename_with_path, table):
    ...
    try:
        ...
        sql = 'copy into {0} from (select c.* from @innercircle/{1} (file_format => D_FILEFORMAT ) c) PURGE =FALSE on_error=continue'.format(table, file[0])
        cs.execute(sql)
        print(f"🌿🌿 copy data from stage into snowflake {table}:" + str(datetime.datetime.now()))
        sql = "create or replace table rejectedrows_{0} as select * from table(validate(ANALYTICS.PUBLIC.{0},job_id=>'_last'))".format(table)
        cs.execute(sql)
...
def df_to_snowflake(df,table):
 ...
 ##to get data from snowflaek internal stage not possible to get directly from s3 to local but possible with snowflake managed stages
