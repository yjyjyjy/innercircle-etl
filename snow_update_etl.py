...
def update_eth_transactions(date):
    # ******* dump raw transaction data into snowflake table *******
...
def update_nft_trade_opensea(date, running_in_cloud=utl.RUNNING_IN_CLOUD, use_upsert=True):
    ...
    utl.copy_from_df_to_snowflake_table(
        df=df,
        table="nft_trades",
        csv_filename_with_path=csv_filename_with_path,
        use_upsert=use_upsert,
...
def update_collection(pagination=5, todo_list=None, update=False):
    if todo_list == None:
        result = utl.query_snowflake(sql="select address from new_nft_contracts where missing_metadata", columns=["id"])
    ...
    while len(todo_list) > 0:
        ...
        if output.shape[0] > 0:
            utl.copy_from_df_to_snowflake_table(df=output, table="collection", use_upsert=True, key="id", update=update)
...
def update_nft_contract_abi(pagination=5):
    ...
    result = utl.query_snowflake(sql="select address from new_nft_contracts where missing_abi", columns=["address"])
    ...
    while len(todo_list) > 0:
        ...
        if output.shape[0] > 0:
            utl.copy_from_df_to_snowflake_table(df=output, table="nft_contract_abi", use_upsert=True, key="address")
        todo_list = [x for x in todo_list if x not in _todo]

""" WE HAVE TO REPLACE THIS PART WITH ANY ONE YOU WANT
...
def update_nft_trx_union(date):
    ...
    ''')
    utl.copy_from_df_to_snowflake_table(df=df, table="nft_trx_union", use_upsert=False)
...
def update_post():
    ...
    todo_list = utl.query_snowflake(sql='''
        select c.id
        from post p
        join collection c
...
def update_address_metadata_trader_profile():
    ...
    utl.query_snowflake(sql='truncate table address_metadata_opensea;')
    utl.copy_from_df_to_snowflake_table(df = output, table='address_metadata_opensea', csv_filename_with_path=None, use_upsert=True, key='id')
    utl.query_snowflake(sql='''
