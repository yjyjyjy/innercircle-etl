...
def connect_snowflake():
    try:
        conn = connect(user=SNOW_USER, password=SNOW_PASSWORD, account=SNOW_ACCOUNT, role=SNOW_ROLE)
...
def copy_from_file_to_snowflake_table(csv_filename_with_path, table):
    ...
    try:
        ...
        sql = "copy into {0} from (select c.* from @innercircle (file_format => D_FILEFORMAT ) c) PURGE =TRUE".format(
            table
        )
        cur.execute(sql)
...
def copy_from_df_to_snowflake_table(df, table, csv_filename_with_path=None, use_upsert=False, key=None, update=False):
    ...
    if use_upsert:
        ...
        upsert_from_file_to_snowflake(
            csv_filename_with_path=csv_filename_with_path, table=table, key=key, update=update
        )
    else:
...
def exports_from_snowflake_to_s3(sql, table):
    ...
    try:
        sql = "copy into '@gets3data/{0}/' from ({1})  file_format=(type=csv COMPRESSION =  NONE)HEADER = TRUE OVERWRITE = FALSE SINGLE = TRUE".format(
            table, sql
        )
        cur.execute(sql)
...
def delete_current_day_data(date, table, key="timestamp"):
    gap = check_table_for_date_gaps(table=table, start_date=date, end_date=date, key=key)

    if len(gap) == 0:  # there is existing data.
        query_snowflake(sql=f"delete from {table} where date({key}) = '{date}'")
